...
const KAMCH_MARKER = "@@KAMCHILIKLAR@@";
const escXml = (s) => String(s).replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;");
// Маркер турган параграфни ҳар бир қатор учун алоҳида параграф (олдида tab) билан алмаштиради.
// Шаблондаги параграфнинг ўз хусусиятлари (шрифт, ўлчам, интервал) сақланади.
function replaceWithParagraphs(xml, marker, lines) {
  ...
  const srcPara = xml.slice(pStart, pEnd);

  // Параграф хусусиятлари (<w:pPr>) — интервал, текислаш ва ҳ.к.
  let pPr = "";
  const pPrM = srcPara.match(/<w:pPr>[\s\S]*?<\/w:pPr>/);
  if (pPrM) {
    // Параграфлар ораси интервал бўлмаслиги учун spacing'ни 0 га мажбурлаймиз
    let props = pPrM[0];
    if (/<w:spacing[^/]*\/>/.test(props)) {
      props = props.replace(/<w:spacing[^/]*\/>/, '<w:spacing w:after="0" w:line="240" w:lineRule="auto"/>');
    } else {
      props = props.replace("<w:pPr>", '<w:pPr><w:spacing w:after="0" w:line="240" w:lineRule="auto"/>');
    }
    // Чекиниш (indent) таб каби кўринади — олиб ташлаймиз, фақат биттагина ўз табимиз қолсин
    props = props.replace(/<w:ind[^>]*\/>/g, "").replace(/<w:ind[^>]*>[\s\S]*?<\/w:ind>/g, "");
    pPr = props;
  } else {
    pPr = '<w:pPr><w:spacing w:after="0" w:line="240" w:lineRule="auto"/></w:pPr>';
  }

  // Run хусусиятлари (<w:rPr>) — шрифт номи ва ўлчами (маркер турган run'дан)
  let rPr = "";
  const runM = srcPara.match(/<w:r(?:\s[^>]*)?>([\s\S]*?)<\/w:r>/);
  if (runM) {
    const rPrM = runM[1].match(/<w:rPr>[\s\S]*?<\/w:rPr>/);
    if (rPrM) rPr = rPrM[0];
  }

  const paras = lines
    .map((l) => `<w:p>${pPr}<w:r>${rPr}<w:tab/><w:t xml:space="preserve">${escXml(l)}</w:t></w:r></w:p>`)
    .join("");
  return xml.slice(0, pStart) + paras + xml.slice(pEnd);
}
...
function buildActData(ins, defects, cfg) {
  ...
  return {
    ...
    kamchiliklar: KAMCH_MARKER,
    _kamchLines: dlist.length ? dlist.map((d, i) => `${i + 1}. ${d.name}`) : ["Камчиликлар аниқланмади"],
    rasmlar: IMG_MARKER,
  };
}
...
async function fetchPhotos(ins) {
  ...
  for (const u of (ins.photos || [])) {
    ...
    try { list.push(new Uint8Array(await fetchBinary(u))); }
    catch (e) { failed++; console.error("Расм юкланмади:", u, e); }
  }
  return { list, failed };
}

// CORS'га чидамли юклаш: аввал тўғридан-тўғри, бўлмаса Vercel proxy (/aw) орқали
function proxied(u) {
  return String(u).replace(/^https:\/\/[^/]+\/(v\d+)\//, "/aw/$1/");
}
async function fetchBinary(u) {
  ...
  throw new Error("Юклаб бўлмади: " + u);
}
...
async function fetchTemplate(cfg) {
  ...
  try { return await fetchBinary(url); }
  catch (e) { throw new Error("Шаблонни юклаб бўлмади."); }
}
function renderTemplate(ab, data) {
  const zip = new PizZip(ab);
  const doc = new Docxtemplater(zip, { paragraphLoop: true, linebreaks: true });
  const { _kamchLines, ...tplData } = data;
  doc.render(tplData);
  // Камчиликлар: ҳар бири алоҳида параграф, олдида tab
  zip.file("word/document.xml", replaceWithParagraphs(zip.file("word/document.xml").asText(), KAMCH_MARKER, _kamchLines || []));
  return zip;
}
const safeName = (s) => (s || "kurik").replace(/[^0-9A-Za-zА-Яа-яЁёЎўҚқҒғҲҳ]+/g, "_").slice(0, 40);

function photoWarning(failed) {
  if (failed > 0) {
    alert(`Диққат: ${failed} та расмни юклаб бўлмади, улар ҳужжатга қўшилмади.\n\nСабаби кўпинча: Appwrite'да Web platform hostname рўйхатига сайт домени қўшилмаган. Appwrite → Overview → Add platform → Web орқали доменни қўшинг.`);
  }
}
...
async function downloadAct(ins, defects, cfg) {
  ...
  const ph = await fetchPhotos(ins);
  const table = addImages(zip, ph.list, { n: 0 });
...
async function downloadActBatch(list, defects, cfg) {
  ...
  const ph0 = await fetchPhotos(list[0]); failedTotal += ph0.failed;
  const t0 = addImages(base, ph0.list, ctr);
  ...
  for (let i = 1; i < list.length; i++) {
    ...
    const phi = await fetchPhotos(list[i]); failedTotal += phi.failed;
    const ti = addImages(base, phi.list, ctr); // медиа/рухсат BASE'га
    ...
    <div style={{ fontFamily: "system-ui, -apple-system, Segoe UI, Roboto, sans-serif", color: C.ink, background: "#eef1f5", minHeight: 640 }}>
      ...
      <main className="app-main" style={{ maxWidth: 980, margin: "0 auto", padding: "18px 16px 60px" }}>
        ...
        {!ready ? (
          ...
          <InspectionForm defects={defects} user={user} editing={editId ? inspections.find((x) => x.id === editId) : null}
...
function InspectionForm({ defects, user, editing, onDone, onCancel }) {
  const e = editing || {};
  ...
  const [inspector, setInspector] = useState(e.inspector || user?.name || user?.email || "");
  ...
  const [photoBytes, setPhotoBytes] = useState(0);          // сиқилгандан кейинги ҳажм
  ...
  async function onFiles(ev) {
    ...
    setBusy(true);
    try {
      // Босқичли сиқиш: катта расмлар 10 МБ чегарасига сиғгунча кучлироқ сиқилади
      const LEVELS = [
        [1280, 0.72],
        [1024, 0.6],
        [800, 0.5],
      ...
      for (const f of files) {
        let d = null;
        for (const [dim, q] of LEVELS) {
          d = await compressImage(f, dim, q);
          if (bytes + dataUrlBytes(d) <= PHOTO_LIMIT_BYTES) break;
        }
        const sz = dataUrlBytes(d);
        if (bytes + sz > PHOTO_LIMIT_BYTES) {
          setPhotoErr("Расмлар жами ҳажми 10 МБ чегарасига сиғмади. Аввалги расмлардан бирини ўчиринг.");
          break;
        }
        added.push(d);
        bytes += sz;
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(90px,1fr))", gap: 8 }}>
            ...
            {photos.length < PHOTO_LIMIT_COUNT && (
              <button onClick={() => setPhotoMenu(true)} disabled={busy} style={{ aspectRatio: "1", border: `2px dashed ${C.line}`, background: C.field, borderRadius: 9, display: "grid", placeItems: "center", color: C.muted }}>
                <div style={{ textAlign: "center" }}><Camera size={20} /><div style={{ fontSize: 11, marginTop: 3 }}>{busy ? "…" : "Расм қўшиш"}</div></div>
              </button>
            )}
          </div>
          ...
          {photoMenu && (
            <Modal onClose={() => setPhotoMenu(false)}>
              <h3 style={{ marginTop: 0, marginBottom: 14 }}>Расм қўшиш</h3>
              <div style={{ display: "grid", gap: 8 }}>
                <button onClick={() => { setPhotoMenu(false); camRef.current?.click(); }}
                  style={{ display: "flex", alignItems: "center", gap: 12, border: `1px solid ${C.line}`, background: C.field, borderRadius: 10, padding: "14px 16px", fontWeight: 700, fontSize: 14.5, textAlign: "left" }}>
                  <Camera size={20} color={C.live} /> Суратга олиш (камера)
                </button>
                <button onClick={() => { setPhotoMenu(false); fileRef.current?.click(); }}
                  style={{ display: "flex", alignItems: "center", gap: 12, border: `1px solid ${C.line}`, background: C.field, borderRadius: 10, padding: "14px 16px", fontWeight: 700, fontSize: 14.5, textAlign: "left" }}>
                  <ImageIcon size={20} color={C.blue} /> Галереядан танлаш
                </button>
              </div>
            </Modal>
          )}
