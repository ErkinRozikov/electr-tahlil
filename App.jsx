...
const DEFAULT_REPORT = {
  orgName: "",
  actTitle: "КЎРИК ДАЛОЛАТНОМАСИ",
  place: "",
  footer: "",
  signatures: ["Комиссия раиси", "Комиссия аъзоси", "Комиссия аъзоси"],
  templateFileId: "",
  templateName: "",
};
const SEED_DEFECTS = [
...
const ACT_TAGS = [
  ["{org_nomi}", "Ташкилот номи"],
  ["{hujjat_nomi}", "Ҳужжат номи"],
...
function buildActData(ins, defects, cfg) {
  ...
  return {
    org_nomi: cfg.orgName || "",
    hujjat_nomi: cfg.actTitle || "",
    joy: cfg.place || "",
    ...
    kamchiliklar: dlist.length
      ? dlist.map((d, i) => `${i + 1}. ${d.name} (${(SEV[d.sev] || SEV.low).label})`).join("\n")
      : "Камчиликлар аниқланмади",
    imzolar: (cfg.signatures || []).join("\n"),
    joy_sana: cfg.place || "",
    // Кенгайтирилган: цикл учун (масалан {#kamchiliklar_royxati}{nomi} - {daraja}{/kamchiliklar_royxati})
    kamchiliklar_royxati: dlist.map((d) => ({ nomi: d.name, daraja: (SEV[d.sev] || SEV.low).label, toifa: d.cat })),
    parametrlar_royxati: params.map((p) => ({ nomi: p.k, qiymat: p.v, birlik: p.u || "" })),
  };
}

async function downloadAct(ins, defects, cfg) {
  if (!cfg || !cfg.templateFileId) {
    throw new Error("Аввал «Далолатнома шакли» бўлимида Word шаблон юкланг.");
  }
  const url = store.templateUrl(cfg.templateFileId);
  const ab = await fetch(url).then((r) => {
    if (!r.ok) throw new Error("Шаблонни юклаб бўлмади.");
    return r.arrayBuffer();
  });
  const zip = new PizZip(ab);
  const doc = new Docxtemplater(zip, { paragraphLoop: true, linebreaks: true });
  doc.render(buildActData(ins, defects, cfg));
  const blob = doc.getZip().generate({
    type: "blob",
    mimeType: "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
  });
  const safe = (ins.name || "kurik").replace(/[^0-9A-Za-zА-Яа-яЁёЎўҚқҒғҲҳ]+/g, "_").slice(0, 40);
  const a = document.createElement("a");
  a.href = URL.createObjectURL(blob);
  a.download = `Далолатнома_${safe}.docx`;
  ...
  return (
    <div>
      <div className="no-print" style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14, flexWrap: "wrap" }}>
        ...
        {cfg.templateFileId && (
          <button onClick={async () => { try { await downloadAct(ins, defects, cfg); } catch (e) { alert("Word яратишда хатолик: " + (e?.message || "шаблонни текширинг")); } }}
            className="ghost" style={{ ...btn, borderColor: C.blue, color: C.blue }}><FileText size={15} /> Word далолатнома</button>
        )}
      ...
      <div className="print-area" style={{ background: C.white, border: `1px solid ${C.line}`, borderRadius: 12, padding: 20 }}>
        {cfg.orgName && (
          <div style={{ textAlign: "center", fontWeight: 700, fontSize: 14, marginBottom: 12, paddingBottom: 12, borderBottom: `1px solid ${C.line}`, whiteSpace: "pre-line" }}>{cfg.orgName}</div>
        )}
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, marginBottom: 4 }}>
          <div>
            <div style={{ fontSize: 11, color: C.muted, fontWeight: 600, letterSpacing: 1 }}>{cfg.actTitle || "КЎРИК ДАЛОЛАТНОМАСИ"}</div>
        ...
        {cfg.footer && (
          <div style={{ marginTop: 18, fontSize: 12.5, color: "#334", whiteSpace: "pre-line" }}>{cfg.footer}</div>
        )}
        {cfg.signatures && cfg.signatures.length > 0 && (
          <div style={{ marginTop: 26 }}>
            {cfg.signatures.map((s, i) => (
              <div key={i} style={{ display: "flex", alignItems: "flex-end", gap: 12, marginTop: 16 }}>
                <span style={{ fontSize: 13, color: C.ink, minWidth: 150 }}>{s}</span>
                <span style={{ flex: 1, borderBottom: `1px solid ${C.ink}`, height: 16 }} />
                <span style={{ fontSize: 11, color: C.muted, width: 120, textAlign: "center" }}>(имзо, Ф.И.Ш.)</span>
              </div>
            ))}
            {cfg.place && <div style={{ marginTop: 18, fontSize: 12.5, color: C.muted }}>{cfg.place}</div>}
          </div>
        )}
      </div>
      {confirm && (
...
function ReportSettings({ cfg, onSave }) {
  const [orgName, setOrgName] = useState(cfg.orgName || "");
  const [actTitle, setActTitle] = useState(cfg.actTitle || "КЎРИК ДАЛОЛАТНОМАСИ");
  const [place, setPlace] = useState(cfg.place || "");
  const [footer, setFooter] = useState(cfg.footer || "");
  const [signatures, setSignatures] = useState(cfg.signatures?.length ? cfg.signatures : [""]);
  ...
  const setSig = (i, v) => setSignatures((s) => s.map((x, idx) => (idx === i ? v : x)));
  const addSig = () => setSignatures((s) => [...s, ""]);
  const rmSig = (i) => setSignatures((s) => s.filter((_, idx) => idx !== i));

  function curCfg(extra) {
    return {
      orgName: orgName.trim(), actTitle: actTitle.trim() || "КЎРИК ДАЛОЛАТНОМАСИ",
      place: place.trim(), footer: footer.trim(),
      signatures: signatures.map((s) => s.trim()).filter(Boolean),
      templateFileId, templateName, ...extra,
    };
  }
  ...
  async function onTemplate(ev) {
    ...
    try {
      ...
      await onSave(curCfg({ templateFileId: id, templateName: file.name }));
      setSaved(true);
  ...
  async function removeTemplate() {
    ...
    await onSave(curCfg({ templateFileId: "", templateName: "" }));
  }

  async function save() {
    setBusy(true); setSaved(false);
    try { await onSave(curCfg()); setSaved(true); }
  ...
  return (
    <div>
      ...
      <p style={{ color: C.muted, fontSize: 13, marginTop: 0 }}>Бу созламалар чоп этиладиган далолатномага (ҳисоботга) татбиқ этилади. Фақат администратор ўзгартира олади.</p>

      <div style={{ background: C.white, border: `1px solid ${C.line}`, borderRadius: 12, padding: 18 }}>
        {/* Word шаблон */}
        <div style={{ marginBottom: 18, paddingBottom: 18, borderBottom: `1px solid ${C.line}` }}>
          <div style={{ fontSize: 13.5, fontWeight: 700, marginBottom: 4 }}>Word шаблон (.docx)</div>
          <div style={{ fontSize: 12, color: C.muted, marginBottom: 10, lineHeight: 1.5 }}>
            Word файлга якорларни (мас: <span className="mono">{"{qurilma_nomi}"}</span>) керакли жойларга ёзинг. Далолатнома яратилганда улар маълумотлар билан тўлдирилади.
          </div>
          {templateName ? (
          ...
          <button onClick={() => setShowTags((v) => !v)} className="ghost" style={{ marginTop: 12, border: "none", background: "transparent", color: C.blue, fontSize: 12.5, fontWeight: 600, padding: 0 }}>
            {showTags ? "▼" : "▶"} Якорлар (placeholder) рўйхати
          </button>
          {showTags && (
            <div style={{ marginTop: 8, background: C.field, borderRadius: 9, padding: 12 }}>
              <div style={{ fontSize: 11.5, color: C.muted, marginBottom: 8 }}>Бу матнларни Word шаблонингизга нусхаланг:</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "6px 14px" }}>
                {ACT_TAGS.map(([tag, label]) => (
                  <div key={tag} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12.5 }}>
                    <span className="mono" style={{ color: C.blue, fontWeight: 700 }}>{tag}</span>
        ...
        <Field label="Ташкилот номи (сарлавҳа)" hint="Ички (HTML) кўриниш ва чоп этишда ишлатилади. Word шаблонда {org_nomi} якори орқали чиқади.">
          <textarea value={orgName} onChange={(e) => setOrgName(e.target.value)} rows={2} placeholder="мас: Ўзэнергоинспекция ... ҳудудий бўлими" style={{ ...inputStyle, resize: "vertical" }} />
        </Field>
        <Field label="Ҳужжат номи">
          <input value={actTitle} onChange={(e) => setActTitle(e.target.value)} placeholder="КЎРИК ДАЛОЛАТНОМАСИ" style={inputStyle} />
        </Field>
        <Field label="Изоҳ / асос (ихтиёрий)" hint="Имзолардан юқорида чиқади (мас: меъёрий ҳужжатга ҳавола).">
          <textarea value={footer} onChange={(e) => setFooter(e.target.value)} rows={2} placeholder="мас: Кўрик ... асосида ўтказилди" style={{ ...inputStyle, resize: "vertical" }} />
        </Field>

        <div style={{ marginBottom: 14 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
            <div style={{ fontSize: 12.5, fontWeight: 600 }}>Имзо қаторлари</div>
            <button onClick={addSig} style={{ display: "flex", alignItems: "center", gap: 5, border: "none", background: C.panel, color: "#fff", borderRadius: 8, padding: "7px 12px", fontWeight: 600, fontSize: 13 }}><Plus size={14} /> Қатор қўшиш</button>
          </div>
          <div style={{ display: "grid", gap: 6 }}>
            {signatures.map((s, i) => (
              <div key={i} style={{ display: "flex", gap: 6 }}>
                <input value={s} onChange={(e) => setSig(i, e.target.value)} placeholder="мас: Комиссия раиси" style={{ ...inputStyle, flex: 1 }} />
                <button onClick={() => rmSig(i)} className="ghost" style={{ border: `1px solid ${C.line}`, background: C.white, borderRadius: 9, padding: "0 11px" }}><X size={16} color={C.muted} /></button>
              </div>
            ))}
          </div>
        </div>

        <Field label="Жой / сана изоҳи (ихтиёрий)">
          <input value={place} onChange={(e) => setPlace(e.target.value)} placeholder="мас: Тошкент ш." style={inputStyle} />
        </Field>

        <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 6 }}>
          <button onClick={save} disabled={busy} style={{ display: "flex", alignItems: "center", gap: 7, border: "none", background: C.live, color: C.panel, borderRadius: 10, padding: "12px 20px", fontWeight: 800, fontSize: 15 }}>
            {busy ? <Loader2 size={17} className="spin" /> : <Save size={17} />} Сақлаш
          </button>
          {saved && <span style={{ color: C.good, fontSize: 13, fontWeight: 600 }}>✓ Сақланди</span>}
        </div>
      </div>
    </div>
  );
