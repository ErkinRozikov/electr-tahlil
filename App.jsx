...
function ReportSettings({ cfg, onSave }) {
  ...
  async function save() {
    ...
    try {
      await onSave({ orgName: orgName.trim(), actTitle: actTitle.trim() || "КЎРИК ДАЛОЛАТНОМАСИ", place: place.trim(), footer: footer.trim(), signatures: signatures.map((s) => s.trim()).filter(Boolean) });
      setSaved(true);
  ...
  return (
    <div>
      ...
      <div style={{ background: C.white, border: `1px solid ${C.line}`, borderRadius: 12, padding: 18 }}>
        <Field label="Ташкилот номи (сарлавҳа)" hint="Далолатнома тепасида марказда чиқади. Бир неча қатор ёзиш мумкин.">
