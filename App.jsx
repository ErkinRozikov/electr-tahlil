...
// Бир нечта кўрик → битта Word (ҳар бири янги саҳифада)
async function downloadActBatch(list, defects, cfg) {
  ...
  const first = splitBody(base.file("word/document.xml").asText());
  let merged = first.content;
  for (let i = 1; i < list.length; i++) {
    const z = renderTemplate(ab, buildActData(list[i], defects, cfg));
    merged += PAGE_BREAK + splitBody(z.file("word/document.xml").asText()).content;
  }
