...
function AuthScreen({ onAuthed }) {
  ...
  async function submit() {
    ...
    if (mode === "register" && password.length < 8) { setErr("Парол камида 8 белгидан иборат бўлсин."); return; }
    setBusy(true);
    try {
      const u = mode === "login"
        ? await auth.login(email.trim(), password)
        : await auth.register(email.trim(), password, name.trim() || email.split("@")[0]);
      onAuthed(u);
    } catch (e) {
      setErr(e?.message || "Хатолик юз берди. Маълумотларни текширинг.");
      setBusy(false);
    }
  }
  ...
  return (
    <div style={{ fontFamily: "system-ui, sans-serif", minHeight: 560, display: "grid", placeItems: "center", background: "#eef1f5", padding: 16 }}>
      ...
      <div style={{ width: "100%", maxWidth: 380, background: "#fff", borderRadius: 16, padding: 26, boxShadow: "0 10px 40px rgba(13,20,28,.08)" }}>
        ...
        <div style={{ display: "flex", background: C.field, borderRadius: 10, padding: 4, marginBottom: 18 }}>
          {[["login", "Кириш"], ["register", "Рўйхатдан ўтиш"]].map(([k, l]) => (
            <button key={k} onClick={() => { setMode(k); setErr(""); }}
              style={{ flex: 1, border: "none", borderRadius: 8, padding: "9px", fontWeight: 700, fontSize: 13.5, background: mode === k ? "#fff" : "transparent", color: mode === k ? C.ink : C.muted, boxShadow: mode === k ? "0 1px 3px rgba(0,0,0,.1)" : "none" }}>
              {l}
            </button>
          ))}
        </div>
        ...
        {mode === "register" && (
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Исм (ихтиёрий)"
            style={authInput} />
        )}
        ...
        <button onClick={submit} disabled={busy}
          ...
          {mode === "login" ? "Кириш" : "Аккаунт яратиш"}
        </button>
      </div>
    </div>
  );
...
function App({ user, onLogout }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!d) { d = SEED_DEFECTS.map(([name, cat, sev]) => ({ id: uid(), name, cat, sev })); await store.setDefects(d); }
      setDefects(d);
      setInspections(await store.listInspections());
      setReady(true);
    })();
  }, []);
  ...
  return (
    <div style={{ fontFamily: "system-ui, -apple-system, Segoe UI, Roboto, sans-serif", color: C.ink, background: "#eef1f5", minHeight: 640 }}>
      ...
      <header className="no-print" style={{ background: C.panel, color: C.white, padding: "12px 18px", display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div style={{ display: "grid", gap: 8 }}>
            {params.map((p, i) => (
              <div key={i} style={{ display: "flex", gap: 6 }}>
                <input list="paramSuggest" value={p.k} onChange={(ev) => setParam(i, "k", ev.target.value)} placeholder="Параметр" style={{ ...inputStyle, flex: 2 }} />
                <input value={p.v} onChange={(ev) => setParam(i, "v", ev.target.value)} placeholder="Қиймат" className="mono" style={{ ...inputStyle, flex: 1.4 }} />
                <input value={p.u} onChange={(ev) => setParam(i, "u", ev.target.value)} placeholder="Бирлик" style={{ ...inputStyle, flex: 1 }} />
                <button onClick={() => rmParam(i)} className="ghost" style={{ border: `1px solid ${C.line}`, background: C.white, borderRadius: 9, padding: "0 11px" }}><X size={16} color={C.muted} /></button>
              </div>
            ))}
